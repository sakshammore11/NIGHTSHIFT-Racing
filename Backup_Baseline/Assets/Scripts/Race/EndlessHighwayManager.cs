...
public class EndlessHighwayManager : MonoBehaviour
{
    ...
    void CreateMaterials()
    {
        ...
        if (roadMaterial == null)
        {
            ...
            roadMaterial.color = new Color(0.1f, 0.11f, 0.14f); // Dark asphalt
        }

        if (borderMaterial == null)
        {
            borderMaterial = new Material(litShader);
            borderMaterial.color = new Color(0.9f, 0.25f, 0.1f); // Red guardrail
        }

        if (mountainMaterial == null)
        {
            mountainMaterial = new Material(litShader);
            mountainMaterial.color = new Color(0.04f, 0.05f, 0.08f); // Dark night mountain silhouette
        }

        if (streetlightMaterial == null)
        {
            streetlightMaterial = new Material(litShader);
            streetlightMaterial.color = new Color(1f, 0.85f, 0.4f); // Warm yellow glow
        }

        if (trafficCarMaterial == null)
        {
            trafficCarMaterial = new Material(litShader);
            trafficCarMaterial.color = new Color(0.8f, 0.1f, 0.1f); // Red AI car
        }
    }
    ...
    void SpawnChunk(bool isSafeZone)
    {
        ...
        // 1. Asphalt Road Segment (15m wide x 60m long)
        GameObject road = GameObject.CreatePrimitive(PrimitiveType.Cube);
        road.name = "Road";
        road.transform.SetParent(chunk.transform);
        road.transform.localPosition = new Vector3(0f, -0.1f, chunkSize * 0.5f);
        ...
        railL.transform.localPosition = new Vector3(-7.6f, 0.4f, chunkSize * 0.5f);
        railL.transform.localScale = new Vector3(0.4f, 0.8f, chunkSize);
        ...
        railR.transform.localPosition = new Vector3(7.6f, 0.4f, chunkSize * 0.5f);
        railR.transform.localScale = new Vector3(0.4f, 0.8f, chunkSize);
        railR.GetComponent<MeshRenderer>().sharedMaterial = borderMaterial;

        // 4. Streetlights (Every 30 meters)
        for (float z = 10f; z < chunkSize; z += 30f)
        {
